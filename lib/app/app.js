...
export function App (sources) {
  ...
  const ipfsPublish$ = ipfsAddPublic$
    .map(files => {
      if (files) {
        ...
        return sources.IPFS.name.publish(publicDir.hash, { t: '1h', ttl: '1h' });
  ...
  const publishNewPost$ = newPost.post$
    ...
    .flatten()
    .startWith(null)
    .debug('publishNewPost$');

  const vtree$ = xs
    .combine(ipfsId$, time$, ipfsAdd$, ipfsConnect$, ipfsAddPublic$, ipfsPublish$, loading.DOM, feed.DOM, newPost.DOM, newPost.post$, publishNewPost$)
    .map(([id, time, add, connect, addPublic, publish, loading, feed, newPost, post, zzz]) => {
      return div([
        newPost,
        pre(stringify(zzz)),
        hr(),
