import { div, h1, h4, hr, pre } from '@cycle/dom';
...
export function App (sources) {
  ...
  const vtree$ = xs
    .combine(ipfsId$, time$, ipfsAdd$, ipfsConnect$, ipfsAddPublic$, ipfsPublish$, loading.DOM, feed.DOM, newPost.DOM)
    .map(([id, time, add, connect, addPublic, publish, loading, feed, newPost]) => {
      return div([
        h1('Yoloist'),
        newPost,
